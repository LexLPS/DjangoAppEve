...
| A02 | Cryptographic Failures | **Pass w/ notes** | TLS enforced to every backend in prod; PBKDF2 password hashing; signed email tokens (salted, 3-day expiry); RS256 webhook verification. Notes: at-rest encryption is an infra task (documented, not enforceable in code); Redis pickle trust (R2). |
...
| A07 | Identification & Authentication Failures | **Pass w/ notes** | Session rotation on login (fixation tested), POST-only CSRF logout, per-IP throttle + cross-IP username lockout, admin MFA, Django password validators, no-enumeration password reset. Notes: duplicate emails allowed at registration (R10); email verification tracked but not enforced (R9). |
...
| R4 | **Order reconciliation gap.** If the local `Order` write fails after `checkoutComplete`, Saleor has an order Eve doesn't; its webhooks are then acknowledged-and-dropped ("unknown order"). | Medium (money/state divergence) | Nightly reconciliation job comparing Saleor orders to local ones; alert on unknown-order webhook log events (already logged). |
